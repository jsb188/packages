...
/**
 * Use in-component visibility state (0 | 1 | 2)
 */

export function useVisible(
  openOnMount: boolean = true,
  onVisible?: (val: number) => void
): [
  0 | .5 | 1 | 2,
  (val: 0 | .5 | 1 | 2) => void
  ] {

  const [visible, setVisible] = useState(0 as 0 | .5 | 1 | 2);
  const allowOpen = useRef(openOnMount);
