...
describe('SheetUI rendering', () => {
	it('renders generic visual data without GraphQL-shaped objects', async () => {
		...
		const topLeftCornerCell = host.querySelector('[data-sheet-top-left-corner-cell="true"]') as HTMLElement | null;
		const rowNumbers = host.querySelectorAll('.sheet_ui_row_number') as NodeListOf<HTMLElement>;
		const rowNumber = rowNumbers[0] || null;
		const firstBodyRowNumber = rowNumbers[1] || null;
		...
		expect(cornerCell?.style.zIndex).toBe('37');
		expect(topLeftCornerCell?.style.left).toBe('0px');
		expect(topLeftCornerCell?.style.top).toBe('0px');
		expect(topLeftCornerCell?.style.position).toBe('sticky');
		expect(topLeftCornerCell?.style.zIndex).toBe('37');
		...
		expect(rowNumber?.style.zIndex).toBe('37');
		expect(firstBodyRowNumber?.style.zIndex).toBe('34');
	...
	it('renders empty select cells as blank text with muted color and no chevron', async () => {
		...
		expect(cell?.className).toContain('single_clicked');
	...
	it('applies non-editing selection guards and default editable hover backgrounds to header cells', async () => {
		const columns = getSheetColumnMetrics([
			createColumn('name'),
			createColumn('status', 'SELECT', {
				options: [
		...
		expect(nameHeader?.className).toContain('bg_main_fd_hv');
		...
		expect(lockedHeader?.className).toContain('bg_main_fd_hv');
