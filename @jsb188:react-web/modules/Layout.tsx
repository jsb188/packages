...
export function AppLayout(p: ReactDivElement & {
  ...
  >
    ...
    ? <SidebarEdgeReveal
      ...
      sidebarEdgeAutoOpenEnabled={sidebarEdgeAutoOpenEnabled}
    />
    : null}
    ...
    <div className='h_f h_spread'>
      ...
      <div className='h_f f z4 v_spread shadow_line_soft rel of bg'>
        {ToolbarComponent}
...
/**
 * App content area
 * NOTE: Always use this as a wrapper for every route
 */
...
export function ModalWrapper(p: ModalWrapperProps) {
  const { ToolbarComponent, domId, className, wrapperClassName, containerClassName, shadowClassName, shadowOutlineStyle, children, closePopOver, onCloseModal, addScrollArea, ...other } = p;
  ...
  return (
    <div
      ...
      className={cn(
        'modal_main_content of',
        shadowClassName ?? `alert_shadow_${shadowOutlineStyle || 'default'}`,
        wrapperClassName ?? 'mw',
        size,
        className
      )}
