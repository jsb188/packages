...
export const SHEET_DISPLAY_RULES_EDITOR_SELECTOR = '[data-sheet-display-rules-editor="true"]';
export const SHEET_CONTEXT_MENU_OVERLAY_SELECTOR = [
	SHEET_GRID_EDITOR_SELECTOR,
	SHEET_COLOR_PICKER_SELECTOR,
	SHEET_DISPLAY_RULES_EDITOR_SELECTOR,
...
/*
 * Return whether one event target is inside the Sheet display rules editor overlay.
 */
export function isSheetDisplayRulesEditorEventTarget(target: EventTarget | null) {
	return Boolean(getSheetEventElementTarget(target)?.closest(SHEET_DISPLAY_RULES_EDITOR_SELECTOR));
}
